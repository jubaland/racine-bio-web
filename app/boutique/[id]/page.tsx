...
export default async function BoutiquePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: profile }, products] = await Promise.all([
    supabase.from('merchant_profiles').select('shop_name, rating_avg, rating_count').eq('user_id', id).maybeSingle(),
    fetchProducts(),
  ]);
  if (!profile) notFound();
  const mine = products.filter((p: any) => p.owner_id === id);
  const region = mine.find((p: any) => p.region)?.region || null;
  return <ShopPage shop={{ id, name: profile.shop_name, region, rating: profile.rating_avg != null ? Number(profile.rating_avg) : null, rating_count: profile.rating_count || 0 }} products={mine} />;
}
