...
export async function GET(request: Request) {
  ...
  const summary = {
    ...
    marginPct: sum(withCost, r => r.revenue) > 0
      ? Math.round((sum(withCost, r => r.margin) / sum(withCost, r => r.revenue)) * 1000) / 10 : null,
    avgPrice: sum(rows, r => r.quantity) > 0 ? Math.round(sum(rows, r => r.revenue) / sum(rows, r => r.quantity)) : 0,
