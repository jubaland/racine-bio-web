...
export default function ProfilePage() {
  ...
  // ── Spending dashboard — computed from orders ──────────────────────────────
  ...
  const maxMonthSpend = Math.max(...last6Months.map(m => monthlyMap[m] || 0), 1);
  const currentMonth = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;

  const prodMap: Record<string, { name: string; qty: number; total: number; image: string | null; unit: string }> = {};
  allItems.forEach(item => {
    const name = item.product_name || item.products?.name || `Produit #${item.product_id}`;
    if (!prodMap[name]) prodMap[name] = {
      name, qty: 0, total: 0,
      image: item.product_image_url ?? item.products?.image_url ?? null,
      unit: item.product_unit || item.products?.unit || 'u',
    };
    prodMap[name].qty += item.quantity;
    prodMap[name].total += item.price * item.quantity;
  });
  const topProducts = Object.values(prodMap).sort((a, b) => b.total - a.total).slice(0, 5);
  const maxProdTotal = topProducts[0]?.total || 1;
  const uniqueProductCount = Object.keys(prodMap).length;
          ...
          {orders.length === 0 ? (
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              <div className="bg-gradient-to-br from-[#ecf4d5] to-[#d6eea0] rounded-2xl p-3 text-center">
                <p className="text-[11px] text-[#526500] opacity-70 mb-1">{t('profile.spending_total', 'Total dépensé')}</p>
                <p className="text-lg font-bold text-[#526500] leading-tight">{totalSpent.toLocaleString()}</p>
                <p className="text-[11px] text-[#526500] opacity-60">Fdj</p>
              </div>
              <div className="bg-gradient-to-br from-[#ecf4d5] to-[#d6eea0] rounded-2xl p-3 text-center">
                <p className="text-[11px] text-[#526500] opacity-70 mb-1">{t('profile.spending_avg', 'Panier moyen')}</p>
                <p className="text-lg font-bold text-[#526500] leading-tight">{avgOrder.toLocaleString()}</p>
                <p className="text-[11px] text-[#526500] opacity-60">Fdj</p>
              </div>
              <div className="bg-gradient-to-br from-[#ecf4d5] to-[#d6eea0] rounded-2xl p-3 text-center">
                <p className="text-[11px] text-[#526500] opacity-70 mb-1">{t('profile.spending_products_diff', 'Produits')}</p>
                <p className="text-lg font-bold text-[#526500] leading-tight">{uniqueProductCount}</p>
                <p className="text-[11px] text-[#526500] opacity-60">{t('profile.spending_different', 'différents')}</p>
              </div>
            </div>

            {/* Graphique 6 mois */}
            <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-3">
              {t('profile.spending_last6', '6 derniers mois')}
            </p>
            <div className="flex items-end gap-2 mb-6" style={{ height: '120px' }}>
              {last6Months.map(ym => {
                const spend = monthlyMap[ym] || 0;
                const barPct = spend > 0 ? Math.max((spend / maxMonthSpend) * 100, 6) : 0;
                const isCurrent = ym === currentMonth;
                return (
                  <div key={ym} className="flex-1 flex flex-col items-center justify-end gap-1" style={{ height: '100%' }}>
                    <div className="w-full flex flex-col justify-end" style={{ height: 'calc(100% - 20px)' }}>
                      {spend > 0 && (
                        <p className="text-[9px] text-center text-gray-400 mb-0.5 leading-none">
                          {spend >= 1000 ? `${(spend / 1000).toFixed(0)}k` : spend}
                        </p>
                      )}
                      <div
                        className={`w-full rounded-t-lg transition-all ${isCurrent ? 'bg-[#526500]' : 'bg-[#a8c800]'} ${spend === 0 ? 'opacity-0' : ''}`}
                        style={{ height: `${barPct}%` }}
                      />
                    </div>
                    <p className={`text-[10px] leading-none ${isCurrent ? 'text-[#526500] font-semibold' : 'text-gray-400'}`}>
                      {monthLabel(ym)}
                    </p>
                  </div>
                );
            ...
            {topProducts.length > 0 && (
              <>
                <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-3">
                  {t('profile.spending_top', 'Top produits')}
                </p>
                <div className="space-y-3">
                  {topProducts.map((prod, idx) => (
                    <div key={prod.name} className="flex items-center gap-3">
                      <span className="text-[11px] font-bold text-gray-300 w-4 flex-shrink-0 text-center">{idx + 1}</span>
                      <div className="w-10 h-10 rounded-xl overflow-hidden bg-[#ecf4d5] flex-shrink-0">
                        {prod.image ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1.5">
                          <p className="text-xs font-semibold text-gray-700 truncate pr-2">{prod.name}</p>
                          <p className="text-xs font-bold text-[#526500] flex-shrink-0">{prod.total.toLocaleString()} Fdj</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <div className="flex-1 bg-[#ecf4d5] rounded-full h-1.5 overflow-hidden">
                            <div
                              className="h-full bg-[#a8c800] rounded-full transition-all"
                              style={{ width: `${(prod.total / maxProdTotal) * 100}%` }}
                            />
                          </div>
                          <p className="text-[10px] text-gray-400 flex-shrink-0 tabular-nums">
                            {prod.qty} {prod.unit}
                          </p>
                        </div>
                      </div>
