...
export default function AdminPromos() {
  ...
  const handleSave = async () => {
    if (!form.badge || !form.title) { setError('Badge et titre sont requis.'); return; }
    setSaving(true);
    setError('');
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800">🏷️ Promotions</h1>
      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {promos.length === 0 && (
            <div className="text-center py-12 text-gray-400 bg-white rounded-2xl border border-[#dde8b0]">Aucune promotion</div>
          )}
          {promos.map(p => (
            <div key={p.id} className="bg-white rounded-2xl border border-[#dde8b0] p-5 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: p.color_start + '30', color: p.color_start }}>
                    {p.badge}
                  </span>
                  <button
              ...
              <div className="flex gap-2 flex-shrink-0">
                <button onClick={() => openEdit(p)} className="text-[#7d9800] hover:text-[#526500] text-xs font-medium">Modifier</button>
      ...
      {showModal && (
        <Modal title={editingId ? 'Modifier la promotion' : 'Ajouter une promotion'} onClose={() => setShowModal(false)}>
          <div className="space-y-4">
            ...
            <FormField label="Badge (ex: SOLDES, -20%) *">
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm text-gray-700">Promotion active (visible sur le site)</span>
            </label>
