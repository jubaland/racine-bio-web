...
// Éditeur de composition d'un panier (formulaire produit admin).
// Un panier est un produit Hornafresh : ici on gère seulement son type (thématique / anti-gaspi),
// sa date de fin (anti-gaspi) et ses composants (produits Hornafresh publiés, non paniers).

export type BundleLine = { product_id: number; quantity: number };
export type BundleForm = { is_bundle: boolean; bundle_kind: 'theme' | 'rescue'; bundle_ends_at: string; items: BundleLine[] };
export const EMPTY_BUNDLE: BundleForm = { is_bundle: false, bundle_kind: 'theme', bundle_ends_at: '', items: [] };

type Candidate = { id: number; name: string; unit: string; price: number; cost_price: number | null; stock_qty: number; status: string; owner_id: string | null; is_bundle?: boolean };

const LOW = 5; // même seuil que les alertes de stock bas
...
export default function BundleField({ value, onChange, products, price, stockQty }: {
  value: BundleForm; onChange: (v: BundleForm) => void;
  products: Candidate[];          // catalogue admin complet (filtré ici)
  price: string; stockQty: string; // prix et stock saisis dans le formulaire (pour l'aperçu)
}) {
  ...
  // Composants possibles : produits Hornafresh publiés, non paniers
  const candidates = useMemo(() => products.filter(p => !p.owner_id && !p.is_bundle && p.status === 'published').sort((a, b) => a.name.localeCompare(b.name)), [products]);
  const byId = useMemo(() => Object.fromEntries(candidates.map(p => [p.id, p])) as Record<number, Candidate>, [candidates]);
  const set = (patch: Partial<BundleForm>) => onChange({ ...value, ...patch });

  const add = (id: number, q: number) => {
    if (!id || !(q > 0)) return;
    const items = value.items.filter(i => i.product_id !== id).concat({ product_id: id, quantity: q });
    set({ items }); setPick(''); setQty('1');
  };
  const suggestLowStock = () => {
    // Anti-gaspi : articles à écouler = stock faible mais non nul, pas encore dans le panier
    const low = candidates.filter(p => (p.stock_qty ?? 0) > 0 && p.stock_qty <= LOW && !value.items.some(i => i.product_id === p.id));
  ...
  return (
    <div className="border border-[#d2e095] rounded-2xl p-4 space-y-3 bg-[#fafff0]">
      <label className="flex items-center gap-3 cursor-pointer">
        <input type="checkbox" checked={value.is_bundle} onChange={e => set({ is_bundle: e.target.checked })} className="w-4 h-4 accent-[#a8c800]" />
        <span className="text-sm font-semibold text-[#526500]">🧺 {t('admin.bundle_enable', 'Ce produit est un panier composé')}</span>
      </label>
      {value.is_bundle && (
        <>
          <p className="text-xs text-gray-500">{t('admin.bundle_hint', 'Le stock saisi plus haut = nombre de paniers. À la commande, le stock de chaque composant est décrémenté ; la disponibilité affichée tient compte des deux. Le coût (marge) est calculé automatiquement. Composants : produits Hornafresh publiés uniquement.')}</p>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              {value.bundle_kind === 'rescue' && (
                <button type="button" onClick={suggestLowStock} className="text-xs font-semibold text-[#c2410c] border border-[#fdba74] bg-white rounded-lg px-2.5 py-1 hover:bg-[#fff7ed]">
                  ♻️ {t('admin.bundle_suggest', 'Ajouter les stocks faibles')} (≤ {LOW})
                </button>
              )}
            </div>
                  ...
                  return (
                    <li key={it.product_id} className="flex items-center gap-2 bg-white border border-[#e8f0d0] rounded-xl px-3 py-2">
                      <input type="number" min="0.1" step="0.1" value={it.quantity}
                        ...
                        className="w-20 border border-[#d2e095] rounded-lg px-2 py-1 text-sm" />
                      <span className="text-xs text-gray-500 w-12 truncate">{p?.unit || ''}</span>
            ...
            <div className="flex gap-2">
              <select value={pick} onChange={e => setPick(e.target.value)} className={selectClass + ' flex-1'}>
